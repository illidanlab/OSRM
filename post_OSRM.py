...
from utils import peft_analytical_init, get_pretrained_latent_features, get_post_analytical_init_weights

parser = argparse.ArgumentParser()
# basic configs
parser.add_argument('--seed', type=int, default=42)
parser.add_argument('--backbone', type=str, default='roberta-large')
parser.add_argument('--load-model-path', default='./load_model_path.json', type=str, help='path to .json file for loading LoRA-fine-tuned model weights')
# train configs
...
parser.add_argument('--max-num-samples', type=int, default=100, help='max number of samples when analytical initialization')
# lora configs
parser.add_argument('--alpha', type=int, default=16)
parser.add_argument('--rank', type=float, default=8)
parser.add_argument('--init', type=str, default='post')
...
task_to_keys = {
    ...
    "mnli": ("premise", "hypothesis"), # acc # code bugs
    "cola": ("sentence", None), # corr
    "stsb": ("sentence1", "sentence2"), # corr # regression tasks
}

pretrained_latent_features = None # for analytical initialization

with open(main_args.load_model_path, 'r') as file:
    load_model_paths = json.load(file)
...
def main(task_name, args):
    ...
    '''Load modules:'''
    load_model_path = load_model_paths[task_name]
    ...
    ptr_model = model_class.from_pretrained(
        model_path,
        device_map="auto",
        use_cache=False,
        torch_dtype=torch.bfloat16,
    ...
    ptr_model.pad_token_id = ptr_model.config.eos_token_id
    ptr_model.config.pad_token_id = ptr_model.config.eos_token_id
    ...
    '''Save path:'''
    init = args.init = 'post'
    save_root = f'./.save_dir/{task_name}/{args.backbone}-r{args.rank}-a{args.alpha}-{init}-fix{args.freeze}-{args.block}-wd{args.weight_decay}-seq{args.max_seq_len}'
    run_name = f'{args.backbone}-{task_name}-a{args.alpha}-r{args.rank}-{init}-{args.freeze}-{args.block}-wd{args.weight_decay}-seq{args.max_seq_len}'
    if args.init.startswith('post'):
    ...
    if not pretrained_latent_features:
        ...
        for external_task in external_tasks:
            external_dataset = load_dataset("glue", external_task, cache_dir='./.cache', trust_remote_code=True)
        ...
        pretrained_latent_features = get_pretrained_latent_features(deepcopy(ptr_model), args.rank, external_dataloaders, target_modules)
    ...
    left_lora_init_weights = get_post_analytical_init_weights(model, ptr_model, right_lora_init_weights, target_modules)
    peft_model = get_peft_model(ptr_model, lora_config).to(torch.bfloat16)
    if args.init.startswith('post'):
        for name, param in peft_model.named_parameters():
            if 'lora_A' in name:
                param.data = right_lora_init_weights[name].to(param.device).to(torch.bfloat16)
            elif 'lora_B' in name:
                param.data = left_lora_init_weights[name].to(param.device).to(torch.bfloat16)
    ...
    with open(filename, 'w') as json_file:
        json.dump(vars(args), json_file, indent=4)  # Use indent=4 for pretty-printing
